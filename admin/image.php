...
<html>
    ...
    <body>
                ...
                if ($_FILES['files']['error'][$f] == 0 && $uploadOk != 0) {
                    if ($_FILES['files']['size'][$f] > $max_file_size) {
                        ...
                        if (move_uploaded_file($_FILES["files"]["tmp_name"][$f], $path . $name) && $uploadOk != 0) {
                            ...
                            $query = "insert into images (category, path, date) values ('$catgory','$path$name','$date')";
                            $result = mysqli_query($conn, $query)or die(mysqli_error($conn));
                            $count++; // Number of successfully uploaded file
                        }
                        if ($count == $total_img) {
                            echo "<script>alert('Upload Success!');window.location.href='image.php';</script>";
                        }
                    }
                }
            }
        ...
        //delete image by date
        if (isset($_POST['delDate'])) {
            $date = $_POST['date'];
            $getPath = "select path from images where date = '$date'";
            $pathresult = mysqli_query($conn, $getPath) or die(mysqli_error($conn));
            if (mysqli_num_rows($pathresult) == 0) {
                echo "<script>alert('No image on $date.');window.location.href='image.php';</script>";
            } else {
                while ($path = mysqli_fetch_row($pathresult)) {
                    unlink(urldecode($path[0]));
                }
                $query = "delete from images where date = '$date'";
                $result = mysqli_query($conn, $query);
                echo "<script>alert('All image of $date has been deleted');window.location.href='image.php';</script>";
            }
        }
        ...
        if (isset($_POST['img']) && is_array($_POST['img'])) {
            foreach ($_POST['img'] as $img) {
                $getPath = "select path from images where id = $img";
                $pathresult = mysqli_query($conn, $getPath) or die(mysqli_error($conn));
                $path = mysqli_fetch_row($pathresult);
                unlink(urldecode($path[0]));
                $query = "delete from images where id = $img";
                        ...
                        <div class="panel-body">
                            ...
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="form-group">
                                    <b style="font-size: 13px;">Category:</b><input class="form-control"  type="text"  name="category"  maxlength="20" />
                                </div>
                                <div class="form-group">
            ...
            <div class="row">
                <div class="col-md-12">             
                            ...
                            <form method="post" action="">
                                ...
                                if ($count_row == 0) {
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        echo "<div class=\"col-md-2\"><div style=\"margin-left:25%; margin-top:30%;\">" . $row["date"] . "</div><label for=\"" . $row["id"] . "\"><img style=\"width:125px; height:125px;\" class=\"img-responsive\" alt=\"" . $row["category"] . "\" src=\"" . $row["path"] . "\"></label>"
                    ...
                    <div class="content-box-large">
                        ...
                        <form method="post" action="">
                            <div class="panel-body">
                                <input class="btn btn-info col-lg-3 " style="margin-left: 10px;"  type="date" name="date" value="<?php echo date('Y-m-d'); ?>">
                                <input class="btn btn-warning col-lg-3" style="margin-left: 10px;" onclick="clicked(event)" type="submit" name="delDate" value="Delete selected date">
                            </div>
                        </form>
